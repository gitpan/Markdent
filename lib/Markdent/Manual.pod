...
your own parser subclasses or roles to create your own dialect of Markdown.
...
This is used by the L<HTMLStream handler|Markdent::Handler::HTMLStream>, for
example. Since it generates a different tag for each event, it makes sense to
have one method per event which knows what tag to generate.
...
may find yourself just needing to extending either the block I<or> the span
parser.

For example, the L<Theory dialect|Markdent::Dialect::Theory> shipped with this
distribution is mostly implemented as a subclass of the L<Standard
dialect's|Markdent::Dialect::Standard> block parser.

See the Theory dialect for an example of how to extend the core Markdown
dialect.
